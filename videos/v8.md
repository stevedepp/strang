...
That's -- so if there's a combination of the rows that gives the zero row, then the same combination of the entries of b must give 0. And this isn't the zero row, that's the zero number. Tthis is another way of saying -- and it is not immediate,
...
columns. I'm only using the pivot columns.
